...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    if (occurrences.isEmpty) List(List())
      ...
      val current = occurrences.head._1
      val currentFrequency = occurrences.head._2
      (for {
        i <- 0 until (currentFrequency + 1)
        os <- rest
      } yield if (i == 0) os else (current, i) :: os).toList
    }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def compute(occurr: Occurrences): List[Sentence] =
      if (occurr.isEmpty) List(List())
      else for {
        comb <- combinations(occurr)
        word <- (dictionaryByOccurrences withDefaultValue List())(comb)
        restSentence <- compute(subtract(occurr, comb))
