...
object Huffman {
   ...
   */
   def times(chars: List[Char]): List[(Char, Int)] = {
     ...
     def compute(charList: List[Char], frequency: List[(Char, Int)]): List[(Char, Int)] = charList match {
      case Nil => frequency
      case x :: xs => compute(xs.filter(_ != x), (x, charList.count(_ == x)) :: frequency)
     }
  ...
  def makeOrderedLeafList(frequencies: List[(Char, Int)]): List[Leaf] =
    frequencies.sortBy(_._2).map(leaf => Leaf(leaf._1, leaf._2))
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case Nil => trees
      case x :: Nil => trees
      case first :: second :: xs => (makeCodeTree(first, second) :: trees.tail.tail)
        .sortBy(weight(_))
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      ...
      def traverse(tree:  CodeTree)(c: Char): List[Bit] = tree match {
        case Leaf(_, _) => List()
        case Fork(left, right, _, _) if chars(left).contains(c) => 0 :: traverse(left)(c)
        case Fork(left, right, _, _) => 1 :: traverse(right)(c)
      }
   ...
   */
    def convert(tree: CodeTree): CodeTable = {
      ...
      def solve(root: CodeTree, code: List[Bit]): CodeTable = root match {
        case Leaf(c, _) => List((c, code))
        case Fork(left, right, _, _) => mergeCodeTables(solve(left, code :+ 0), solve(right, code :+ 1))
      }
