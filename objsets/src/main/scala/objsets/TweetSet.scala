...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      val filterChildren = left.filterAcc(p, right.filterAcc(p, acc))
      if (p(elem))
        filterChildren incl elem
      else
        filterChildren
    }
    ...
    def mostRetweeted: Tweet = {
      ...
      if (elem.retweets > leftBest.retweets && elem.retweets > rightBest.retweets) elem
      else if (leftBest.retweets > rightBest.retweets) leftBest
      else rightBest
    }

    def descendingByRetweetAcc(acc: TweetList): TweetList = {
      val best = this.mostRetweeted
      new Cons(best, this.remove(best).descendingByRetweet)
    }
...
object GoogleVsApple {
  ...
  def googleFilter(t: Tweet): Boolean = google.exists((s: String) => t.text.contains(s))
  def appleFilter(t: Tweet): Boolean = apple.exists((s: String) => t.text.contains(s))
